 ...
 */
package com.credit.common.util.security;
...
/**
 * Base64编解码实现。 <p> 将原先使用的Apache项目中的base64类换为Bouncy
 * Castle(www.bouncycastle.org)中的类 Bouncy
 * Castle对byte[]数据的编解码速度比原先的类稍好一点点，并直接提供了String->byte[]
 * 的decode支持，对于带回车符的字符串，如果用原先的类+StringUtil.replaceAll实现相同功能， 速度提高了10倍，即使用jdk1.4
 * String.replaceAll，速度也提高了3倍。 <p> 另外修正了忽略字符时的bug。
 * 
 * @author xuji
 * @version 0.9
 * @since 0.9
...
public class Base64
{
    ...
    public static int decode(String data, OutputStream out) throws IOException
    {
        ...
        /*
         * for (int i = 0; i < end - 4; i += 4) { if (ignore(data.charAt(i))) {
         * continue; } b1 = decodingTable[data.charAt(i)]; b2 =
         * decodingTable[data.charAt(i + 1)]; b3 = decodingTable[data.charAt(i +
         * 2)]; b4 = decodingTable[data.charAt(i + 3)]; out.write((b1 << 2) |
         * (b2 >> 4)); out.write((b2 << 4) | (b3 >> 2)); out.write((b3 << 6) |
         * b4); length += 3; }
         */
